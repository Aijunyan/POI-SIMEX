...
sim.lognormT <- function(a, b, n, 
  ...
  W <- rpois(n, lambda*A) #generate Poisson, mis-measured covariate
  lambda.naive <- W/A
  ...
  simdata <- data.frame(Y = Y,
                        Z = Z,
                        lambda = lambda,
                        lambda.naive = lambda.naive,
                        A = A,
                        delta = delta)
  return (simdata)
}
...
linearextrapolation <- function(A1, A2, A3, lambda) {
  # A1 estimates obtained from each level of lambda.
...
POI.simexaft <- function(formula = formula(data),
                         data = parent.frame(),
                         SIMEXvariable,
                         areaVariable,
                         B = 50,
  ...
  # dist, distribution used in AFT model, current implementation is only for lognormal
  ...
  ##### specifies the regression form used in the extrapolation step #####
  ...
  while(k <= length(lambda)) {
    ...
    for(r in 1:B) { 
      ...
      ### use survreg to fit the artificial simulated data to the the AFT model ###
  ...
  erg <- list(coefficients = estimate,
              se = se,
              scalereg = scalereg,
              pvalue = pvalue,
              lambda = lambda, 
