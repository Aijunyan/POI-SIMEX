###2023-03-12
##simulation of lognormal survival data
...
n<-50;
N=1000;a=1;b=2;b0=2;b1=1;b2=0.5;sig.e=2
my.dat<-sim.lognormT(a,b,N, n,unif.a0=0.5,unif.b0=9,seed=12345,rcensor=0.2)

##step 2 run the analysis
true.est<-c();naive.est<-c(); simex.est<-c()
for (i in 1 :N){
   #i=1
    mydata<-my.dat[my.dat$simseq==i,]
  
    ####true fit
    formula<-Surv(Y,delta) ~ lambda + Z
    
    true.fit = survreg(formula=formula,data=mydata,dist='lognormal',robust=T)
    true.est0<-summary(true.fit)$table
    para<-row.names(true.est0)
    true.est_<-cbind(true.est0,para,simseq=i)
    ...
    naive.est_<-cbind(naive.est0,para,simseq=i)
    ...
    simex.est_<-cbind(para=row.names(simex0),simex0,simseq=i)
    
    true.est<- rbind(true.est,true.est_)
    naive.est<- rbind(naive.est,naive.est_)
    simex.est<- rbind(simex.est,simex.est_)
}

write.csv(true.est,row.names=F,file="./output/true.est.csv")
